...
constructor(
  ...
  ChecarEntrada() {
    ...
    var retraso = moment().subtract(8, "hours").format('HH:mm:ss').toString(); // calculo para sacar los minutos de retraso ///debera implementarse para cuando sea menor a las 8:00 am
    //codigo de periodo de semana 
    var now = moment();
    var monday = now.clone().weekday(1).toString(); //Monday
    var sunday = now.clone().weekday(7).toString(); //Sunday

    //query de envio de datos x medio del AngularFirestoreDocument
    const userRef: AngularFirestoreDocument<any> = this.afs.collection(year.getFullYear().toString()).doc(mesActual).collection('Semana'+semaMo).doc(user.displayName);
    ...
    return userRef.set(tarjeta, {
      merge: true
    }).then(() => { //aqui mandamos el popup del checado
      Swal.fire({ 
        title: 'Registrado!',
        text: 'tu checada de Entrada a sido exitosa  -  latitud:'+this.LAT+" - longitud:"+this.LON,
        imageUrl: './assets/mapa.png', // TODO --- implementar mapa con LAT y LON
        imageWidth: 400,
        imageHeight: 200,
